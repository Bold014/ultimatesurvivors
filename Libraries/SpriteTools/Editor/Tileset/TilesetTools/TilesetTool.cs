...
public partial class TilesetTool : EditorTool
{
	...
	public override void OnUpdate ()
	{
		...
		if ( viewport?.SceneView?.Tools?.CurrentTool?.CurrentTool is null )
		{
	...
	void InitGrid ()
	{
		var viewport = SceneViewWidget.Current?.LastSelectedViewportWidget;
		if ( viewport is null ) return;
		WasGridActive = viewport.State.ShowGrid;
		viewport.State.ShowGrid = false;
	}
	...
	void ResetGrid ()
	{
		var viewport = SceneViewWidget.Current?.LastSelectedViewportWidget;
		if ( viewport is null ) return;
		viewport.State.ShowGrid = WasGridActive;
	}
