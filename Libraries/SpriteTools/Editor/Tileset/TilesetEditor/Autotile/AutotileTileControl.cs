...
public class AutotileTileControl : Widget
{
	...
	public override void OnDragDrop ( DragEvent ev )
	{
		...
		var droppedTile = ev.Data.OfType<TilesetResource.Tile>().FirstOrDefault();
		if ( droppedTile is null ) return;

		Tile.Tiles ??= new System.Collections.Generic.List<AutotileBrush.TileReference>();

		var tileRef = new AutotileBrush.TileReference( droppedTile.Id )
		{
			Tileset = droppedTile.Tileset,
			Position = droppedTile.Position
		};
		Tile.Tiles.Add( tileRef );

		ParentBrush?.ParentList?.MainWindow?.inspector?.UpdateSelectedAutotileSheet();
		ParentBrush?.ParentList?.MainWindow?.SetDirty();
	}
