...
internal sealed class TilesetSceneObject : SceneCustomObject
{
	...
	public override void RenderSceneObject ()
	{
...

		{
			...
			var size = Layer.TilesetResource.GetTileSize();
			...
			// Find the active main camera — prefer the highest-priority one (e.g. PlayerCamera at priority 10)
			CameraComponent bestCam = null;
			foreach ( var c in Component.Scene?.GetAllComponents<CameraComponent>() ?? Enumerable.Empty<CameraComponent>() )
			{
				if ( !c.IsMainCamera || !c.IsValid() ) continue;
				if ( bestCam is null || c.Priority > bestCam.Priority )
					bestCam = c;
			}
			var camWorldPos = bestCam?.WorldPosition ?? Component.Scene?.Camera?.WorldPosition ?? Vector3.Zero;

			if ( size.x > 0 && size.y > 0 )
			{
				int camTileX = (int)MathF.Floor( camWorldPos.x / size.x );
				int camTileY = (int)MathF.Floor( camWorldPos.y / size.y );
				cullMinX = camTileX - TileCullRadiusX;
				cullMaxX = camTileX + TileCullRadiusX;
				cullMinY = camTileY - TileCullRadiusY;
				cullMaxY = camTileY + TileCullRadiusY;
				doCull = true;
			}

			// Max tiles that can appear in the cull window
			const int MaxCullTiles = ( TileCullRadiusX * 2 + 1 ) * ( TileCullRadiusY * 2 + 1 );
			var vertex = ArrayPool<Vertex>.Shared.Rent( doCull ? MaxCullTiles * 6 : Math.Max( Layer.Tiles.Count, 1 ) * 6 );
			...
			foreach ( var tile in Layer.Tiles )
			{
				...
				// Skip tiles outside the camera's visible area
				if ( doCull && ( pos.x < cullMinX || pos.x > cullMaxX || pos.y < cullMinY || pos.y > cullMaxY ) )
					continue;
			...
			if ( i > 0 )
				Graphics.Draw( vertex, i, combo.Item2, Attributes );
			ArrayPool<Vertex>.Shared.Return( vertex );

			maxPosition += new Vector3( size.x, size.y, 0 );
